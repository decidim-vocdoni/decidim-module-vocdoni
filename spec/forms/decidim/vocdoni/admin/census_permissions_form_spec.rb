...
module Decidim::Vocdoni::Admin
  describe CensusPermissionsForm do
    subject { described_class.from_params(verification_types:).with_context(context) }
    ...
    let(:context) { double("Context", current_organization:) }

    before do
      users.each do |user|
        create(:authorization, name: verification_types[0], user:)
        create(:authorization, name: verification_types[1], user:)
      end

...
describe "Admin manages election steps", :slow, type: :system, billy: true do
  ...
  describe "setup an election" do
    let(:election) { create :election, :with_photos, :ready_for_setup, component: current_component, title: { en: "English title", es: "" } }
    ...
    it "performs the action successfully" do
      ...
      within "form.create_election" do
        ...
        expect(find("textarea#setup_demo_census").value.split(/\n/).length).to eq 5

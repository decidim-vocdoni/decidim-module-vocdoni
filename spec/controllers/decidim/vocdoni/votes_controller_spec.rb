...
describe Decidim::Vocdoni::VotesController, type: :controller do
  ...
  describe "POST check_verification" do
    ...
    it "returns verification status" do
      ...
      expect(response.body).to eq({ isVerified: false, email: nil, election_id: election.id, token: nil }.to_json)
    end
    ...
    context "when user is verified and is in the Vocdoni census" do
      ...
      it "returns verification status" do
        post :check_verification, params: { election_id: election.id }
        expect(response).to have_http_status(:success)
        expect(response.body).to eq({ isVerified: true, email: voter.email, election_id: election.id, token: voter.token }.to_json)
      end
    end
    ...
    context "when user is verified and is not in the Vocdoni census" do
      ...
      it "returns verification status" do
        post :check_verification, params: { election_id: election.id }
        expect(response).to have_http_status(:success)
        expect(response.body).to eq({ isVerified: false, email: voter.email, election_id: election.id, token: voter.token }.to_json)
      end
    end
  end
